...
// See associated document on assignment for details of this algorithm
	...
	public static void main(String[] args) {
		...
		while ((p == q) || ((p * q) < 256)) {
			...
			while (!foundP) {
				...
				if ((i >= Math.sqrt(p))) {
					foundP = true;
				} else { // We stopped early since something divided P evenly OR P was 0 or 1
					// Generate a new candidate P
					rand = Math.random();
			...
			while (!foundQ) {
				...
				if ((i >= Math.sqrt(q))) {
					foundQ = true;
				} else { // We stopped early since something divided Q evenly OR Q was 0 or 1
					// Generate a new candidate Q
					rand = Math.random();
		...
		//******************************************************************************
		// Encrypt the message using only the public key (E, PQ)
		//******************************************************************************
